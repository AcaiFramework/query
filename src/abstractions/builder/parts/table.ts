...
export default abstract class TableClass<T = Record<string, ModelContent>> extends QueryClass<T> {
	...
	public async alterTable (columns: Record<string, ColumnOptions>, smartUpdate = true) {
		return await this.getAdapter().alterTable(this.tableName, columns, smartUpdate);
	}
